...
<div class="pd-x-20 pd-sm-x-30 pd-t-20 pd-sm-t-30">
    <h4 class="tx-gray-800 mg-b-5">State Hospitals</h4>
    <p class="mg-b-0">Get all SriLankan State Hospitals</p>
</div>
...
<div class="br-pagebody">
    ...
    <div class="row pd-t-20">
        ...
        <div class="col-lg-4 mg-t-20 mg-lg-t-0">
            <div class="form-group has-warning mg-b-0">
                <select class="form-control select2" data-placeholder="Choose Hospital" name="hospital" id="hospital">
                </select>
            </div>
        </div>
...
<script type="text/javascript">
    $('select[name="province"]').on('change', function() {
        ...
        if (province) {
            $.ajax({
                url: "{{ config('app.url') }}/get/local/district/province/" + province
                , type: "GET"
                , dataType: "json"
                , beforeSend: function() {
    ...
    $('select[name="district"]').on('change', function() {
        ...
        if (district) {
            $.ajax({
                url: "{{ config('app.url') }}/get/local/authority/district/" + district
                , type: "GET"
                , dataType: "json"
                , beforeSend: function() {
                    $('#loader').css('visibility', 'visible');
                ...
                , success: function(data) {
                    ...
                    $('select[name="hospital"]').append('<option value="">Select a Hospital</option>');
                    $.each(data, function(key, value) {
                        $('select[name="hospital"]').append('<option value="' + value + '">' + value + '</option>');
                    });
                }
                , complete: function() {
                    $('#loader').css('visibility', 'hidden');
